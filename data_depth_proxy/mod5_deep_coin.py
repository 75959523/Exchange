...
logger = setup_logger("deep_coin_depth_proxy", os.path.join(project_root, 'log', 'app.log'))
        ...
        try:
            with httpx.Client(proxies=proxy, verify=False, timeout=20) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"deep_coin Request failed with status code {response.status_code}")

                if retry < retry_limit - 1:
                    logger.info(f"deep_coin Retry {retry + 1} for {symbol_name} ...")
